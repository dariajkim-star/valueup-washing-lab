...
### Story 6.4: 순위 불가의 두 범주 분리 (FR15) — 기준축 승격 후속 2026-08-07
As a 애널리스트, I want 순위에서 빠진 종목이 **회사가 안 낸 것**인지 **우리가 못 읽은 것**인지 구분해 보는 것, So that 전자는 판단 재료로 쓰고 후자는 우리 백로그로 보낸다.

**AC**:
- 순위 불가 종목이 **미공시 / 비가독 / 예고 / 무언급** 넷 중 하나로 판정되고, 근거가 함께 노출된다.
- **미공시는 신호다** — 조용히 빠지지 않고 화면에서 *"회사가 목표를 제시하지 않음"*으로 드러난다.
- **비가독은 우리 한계다** — 계속 벌점 없이 제외되고 첨부 취득 워크리스트로 간다.
- **예고는 미공시가 아니다** — 워크리스트가 부르지 않고 화면이 *"계획 예고됨"*으로 말한다(OQ-4 종결 2026-08-07).
- **무언급은 판정 보류다** — 근거 없이 미공시로 세지 않는다(NFR2).
- 판정은 **단일 정의처**를 통한다(`declares_no_attachment`와 나란히). 복제하면 갈라진다.
- `execution_score`·`opacity_rank` **수치는 바뀌지 않는다** — 순위·문구·워크리스트만.

> **왜 지금 나뉘는가.** 리드 결정(2026-08-07)으로 **누락이 기준축**이 됐다(PRD §1.1).
> 기준축이 되면 이 둘은 같은 칸에 있을 수 없다 — 전자는 **찾던 신호 그 자체**이고
> 후자는 **우리 파이프라인의 한계**다. `is_unrankable`의 `count==4` 규칙은 되돌리지 않는다;
> 그 규칙이 막은 오탐(삼성화재·LG화학·하나금융)은 여전히 유효하다.
>
> **착수 전 재측정이 명세를 정정했다(2026-08-07)**: PRD 초안은 비가독을 **40건(상한)**으로
> 적었는데, 그 수는 `게재`·`관련 웹페이지`·`related_url`을 근거에 넣어 나온 것이었다 —
> **셋 다 463/519의 표준 서식 필드로 변별력이 0이다**(2026-07-28에 참조 검사를 폐기한
> 바로 그 이유). 게다가 `첨부서류`는 선언 문구 `첨부서류를 생략한`의 부분문자열이라
> 공존 9건을 만들어냈다. 두 오류를 걷어낸 실측이 **110 / 29 / 67**이다.
>
> **같은 날 오후, 넷이 됐다(OQ-4 종결).** 무언급 67건 중 **56건이 예고(안내공시)**였다 —
> "판정 보류"라던 통의 84%가 실은 애매하지 않았고 우리가 제목을 안 봤을 뿐이다.
> 최종 **110 / 29 / 56 / 11**. 여기서도 변별력 함정이 있었다: `예고(안내공시)` 문자열은
> 104건에 있는데 48건은 **진짜 계획이 자기 예고를 관련공시로 나열한 것**이라,
> 단순 포함으로 잡으면 `axis_targets` 40건이 예고로 뒤집힌다. 제목 서식만 인정해 갈랐다.
