...
> **기준축 (리드 결정 2026-08-07).** 원 painpoint는 *"재무제표에 의도적으로 **누락된** 밸류에이션 지표 때문에 기업 가치가 왜곡된다"*이다. **누락이 기준이고 불이행은 파생이다** — 불이행은 누락하지 않은 기업에만 물을 수 있으므로, 불이행만 재면 아무것도 공시하지 않은 기업이 구조적으로 가장 안전해진다(실측: `valueup_plan` 519건 중 `execution_score` 산출 175건 = 34%). 따라서 CAP-11(불투명도)은 CAP-4·5(이행 갭)의 부속이 아니라 **동급 이상의 기준축**이며, 순위 불가는 *미공시(신호) / 비가독(우리 한계) / 무언급(보류)* 세 범주로 갈린다. 상세는 PRD §1.1 · FR-15.
...
- **CAP-11** 공시 불투명도 — 누락의 계량 · **소급 신설 2026-08-07**
  - **intent:** 공시하지 않은 목표 축의 수를 같은 업종 peer 대비 백분위로 순위화하고(`opacity_rank`), 순위를 매길 수 없는 공시는 그 **사유**를 *미공시 / 비가독 / 무언급*으로 가른다. 고의를 판정하지 않고 **격차를 드러낸다**.
  - **success:** 4축 미공시 수가 peer 백분위 **순위**로 노출되고(점수 아님), 목표를 못 읽은 종목은 벌점 없이 제외되며, 제외 사유가 *회사가 안 냈다 / 우리가 못 읽었다 / 근거 없음*으로 구분돼 함께 제공된다.

> **⚠️ 이 CAP은 2026-08-07에 소급 신설됐다.** epic 6(2026-07-23~28)이 구현하고 화면 기본
> 정렬까지 바꿔놓은 능력이 **SPEC의 Capabilities 목록에 없었다** — CAP-1~10은 2026-07-08자
> 원판 그대로였다. 같은 날 위 §Why에 *"CAP-11(불투명도)"*이라 쓰면서 존재하지 않는 번호를
> 참조한 것이 그 사실을 드러냈다. Epic 6에 스토리 파일이 없던 것(2026-08-06 적발)과
> **같은 계열의 구멍**이고, 이번엔 정본 계약에서 났다.
>
> 원 painpoint가 '누락'인데(§Why 기준축) 그 누락을 재는 능력이 계약에 없었다는 것은
> 단순 누락이 아니라 **문서가 이 프로젝트를 잘못 설명하고 있었다**는 뜻이다.
> FR-13·14·15가 이 CAP을 실현한다.
