...
- 2026-07-10: GPT 2차 교차검증(10건, 대부분 기존 triage와 중복) 중 **신규 실이슈 2건 patch**: (1) **음수/0 분모가 스크리너 오염** — `NULLIF(0)`만으론 자본잠식·적자 기업이 `min_roe` 필터를 통과(roe 부호 뒤집힘). roe·roa·pbr·per·ev_ebitda·debt_ratio·payout_ratio를 `CASE WHEN 분모>0` 가드로 변경(음수 분모→NULL). (2) **NaN/inf 필터값 차단** — 수치 필터에 `allow_inf_nan=False`(422). 회귀 테스트 3건 추가(자본잠식 NULL·min_roe 제외·nan/inf 422) → **pytest 52 passed**. Deferred 2건(마이그레이션 불변성·total/items 스냅샷), 나머지 GPT 지적은 기존 patch/defer/사용자결정과 중복 또는 유니크제약으로 반증. Status: done 유지.
