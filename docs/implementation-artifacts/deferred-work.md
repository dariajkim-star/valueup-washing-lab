...
## Deferred from: code review of 1-11-axis-zero-parser-audit (2026-08-07)

- **`_REFILING_REF`가 `정기공시`·`분기공시`·`제3기공시`에 부분일치** — `(?:旣|기)\s*공시`라
  어절 경계가 없다. 재현: `'정기공시(2026.2.6)에 기재'` → `referenced_date='2026-02-06'`.
  정규식 자체는 1.11 이전 것이고 **1.11은 노출면만 넓혔다** — 이전엔 축=0 문서에서만 돌던
  추출이 이제 `axis_targets` 313건 전부에서 돈다. 코퍼스 실측 오탐 0건.
  **트리거: `axis_targets` 문서에서 `body_reference_date` 오탐이 1건이라도 관측될 때**,
  또는 `choose_plan` 폐기 경로(리뷰 Decision 3)를 손볼 때 함께.
- **plan 137이 이행실적 127%를 목표로 채택 중** — `_RETURN_RE`+`_TARGET_MARK`가
  `"- 주주환원율 127% (주주환원율 목표 35% 이상)"`에서 **뒤따르는 `목표`를 표지로 읽었다**
  (정답 35). 1.11 소관 아닌 기존 결함. 아이러니하게 1.11의 `_premark_gap`은 이 문서에서
  35를 정확히 뽑지만 폴백 순서상 도달하지 못한다 — 폴백 순서를 손볼 때(리뷰 Decision 1,
  선택지 ③) 부수적으로 고쳐질 수 있다. **트리거: 환원율 축 정확도 점검 시 1순위 샘플.**
- **개행 tier가 범위 표현을 못 읽는다(커버리지 공백, 오탐 아님)** — `'ROE \n- 중장기 목표
  10~13%'` → `None`. `_newline_gap`의 gap이 숫자를 금지해 `_PCT`가 범위 앞 숫자에서 끊기고,
  `_RETURN_RANGE_WIDE_RE`는 `_plain_gap`(개행 금지)을 쓴다. 서울보증형 불릿 서식 + 범위
  공시의 조합은 여전히 통째로 버려진다. P1-2가 고친 것과 같은 뿌리.
  **트리거: 축=0 재스캔에서 이 조합의 실샘플이 나올 때** — 지금은 실증 0이므로 규칙을
  넓히지 않는다(관념적 개선 금지).
- **wide tier의 절 순서 의존 — 실적 범위가 목표 범위보다 앞서면 뒤집힌다** — `.search`는
  첫 매칭이라, 한 문서에 이행실적 절이 계획 절보다 먼저 오고 둘 다 범위 표기면 실적을
  목표로 채택한다. 재현: `'2. 2025년 … 이행 현황 \n- 주주환원율 55~57% \n1. 2026년 … 계획
  \n- 주주환원율 20~50% \n- 주주환원 확대'` → 55.0(기대 20.0). 한솔 215는 계획 절이 먼저
  와서 통과할 뿐이다. **닫으려면 절 구조(계획 vs 이행현황) 인식이 필요**해 정규식 층에서는
  풀리지 않는다. "범위는 실적으로 안 쓴다"는 wide tier의 전제가 깨지는 지점.
  **트리거: 이행현황을 앞에 쓰는 공시가 실측될 때**, 또는 본문 절 구조 파싱을 도입할 때.
