...
1. **execution_score 커버리지가 낮다(실데이터 33종목 중 non-null 12종목)** — 원인은 코드가 아니라 원천 데이터: 밸류업 공시의 목표치가 자유서식(범위·서술형)이라 target 파싱이 구조적으로 어렵습니다. 파서 튜닝으로 24%→42%까지 개선했고, v2에서 주주환원율을 별도 축으로 받아 1→12종목으로 늘렸습니다. **낮은 커버리지를 숨기지 않고 정직하게 노출하는 쪽을 설계 원칙으로 채택**했습니다.
...
7. **불투명 4축이 일부 기업의 약속을 담지 못한다** — 척도는 ROE·환원율·기간·자사주계획을 셉니다. 그런데 SK하이닉스(CapEx/Revenue 30% 중반)·LG에너지솔루션(매출 2배·EBITDA Margin 10% 중반)은 **다른 지표로 명확히 약속**했습니다. 이들이 "순위 불가"로 표시되던 것은 부실 공시가 아니라 **우리 자에 눈금이 없는 것**이었습니다. 축을 넓히면 종목 간 비교가 흐려지므로, 유니버스 33종목 규모에서는 **못 재는 것을 못 잰다고 표시하는 쪽**을 택했습니다(화면이 그 이유를 문장으로 말합니다).
