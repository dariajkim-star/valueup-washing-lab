...
- 워싱 판정은 Kleene 3치 논리 — 소각을 확정한 기업은 진척률을 몰라도 확정 False, 근거가 부족하면 True/False가 아니라 **null("판단 불가")**. 실데이터 **359종목**에서 False 80 / 판단 불가 266 / True 2로 설계대로 작동합니다. (`washing_flag`는 2026-07-28 파티에서 **은퇴**했고 — 실측상 켜지지 않는 경고등이었습니다 — 공시 격차를 peer 백분위로 재는 `opacity_rank`가 그 자리를 대신합니다. 컬럼은 계약 호환을 위해 남아 있습니다.)
...
테스트: `pytest -q`(백엔드 377) · `cd dashboard && npm test`(프론트 91) · 마이그레이션 `alembic upgrade head`(0001~0021).
...
에픽별 회고: [Epic 1](docs/implementation-artifacts/epic-1-retro-2026-07-10.md) · [Epic 2](docs/implementation-artifacts/epic-2-retro-2026-07-13.md) · [Epic 3](docs/implementation-artifacts/epic-3-retro-2026-07-14.md) · [Epic 4·5·6 통합](docs/implementation-artifacts/epic-4-5-6-retro-2026-07-28.md)

## 한계와 판단 (의도적으로 안 한 것들)

포트폴리오 프로젝트로서, 리스크를 식별하고도 **비용 대비 판단으로 미구현을 선택한 항목**을 명시합니다. 상세는 [deferred-work](docs/implementation-artifacts/deferred-work.md).

1. **execution_score 커버리지가 낮다(실데이터 359종목 중 non-null 153종목)** — 원인은 코드가 아니라 원천 데이터: 밸류업 공시의 목표치가 자유서식(범위·서술형)이라 target 파싱이 구조적으로 어렵습니다. 파서 튜닝으로 24%→42%까지 개선했고, v2에서 주주환원율을 별도 축으로 받아 커버리지를 늘렸습니다. **낮은 커버리지를 숨기지 않고 정직하게 노출하는 쪽을 설계 원칙으로 채택**했습니다.

   > 📈 2026-07-31 갱신: 유니버스를 **33 → 359종목**으로 확대했습니다(밸류업 계획 제출사 전수 — DART 공시검색 스윕). 같은 날 커버리지를 막던 두 원인을 찾아 고쳤습니다: ① 계획 기간이 없으면 ROE 축뿐 아니라 **점수 전체**가 null이 되던 문제(이제 그 축만 제외하고 채점하며, 제외 사실을 `excluded_axes`로 노출) ② 자사주 표의 `'-'`(활동 없음)를 미상으로 처리해 **"약속하고 실행하지 않은" 기업이 채점에서 빠지던** 문제. 채점 84 → **153종목**. 남은 미채점의 최대 원인은 **계획 기간 파싱(14.9%)**으로, 백로그 P1-8입니다.

   > ⚠️ 2026-07-23 정정: 이 항목은 원래 "워싱 판정의 핵심 신호(washing_flag)는 19종목에서 정상 작동합니다"로 끝났습니다. **그 문장은 틀렸습니다.** 실 DB 적재 후 측정하니 `washing_flag=true`가 **0건**이었습니다 — 4치 AND 조건이 전항 확정 True가 될 일이 목표 null 비율(target_roe 58%·buyback_planned 48%) 아래에서는 거의 없는 구조였습니다. 켜질 수 없는 경고등이었으므로 **지표를 은퇴시키고** 공시 불투명도(`opacity_rank`)로 대체했습니다. 고의를 판정하는 대신 공시 격차를 드러냅니다.

6. **밸류업 계획 실물이 DART 밖에 있다 (수집 사각지대)** — 대기업 다수가 공시 본문에 "상세한 내용은 첨부된 계획을 참고하시기 바랍니다"만 쓰고 실물을 밖에 둡니다. 두 경로를 모두 실증했습니다.
   - **DART 첨부 — 접근 불가·실물 부재**: `robots.txt`가 뷰어·다운로드 경로를 전부 `Disallow`하고(자동 취득 안 함), 사람이 직접 받아봐도 나오는 것은 본문 통지문 HTML 하나뿐이었습니다(3건 확인).
   - **회사 IR 페이지 — 부분 성공**: 공시의 '관련 웹페이지' 필드를 따라갔고 `robots.txt`는 전부 허용이었습니다. 계획서 PDF 직링크를 준 LG화학 1건은 자동 취득·파싱에 성공했습니다(ROE 10.0%·배당성향 20.0%, 근거 p.12). 나머지는 다운로드가 JS 이벤트로만 걸려 있어 정적 HTTP로는 URL을 알 수 없습니다 — 사이트별 어댑터가 필요해 착수하지 않았습니다.
   - **대응**: 사람이 받아 `attachments/`에 두면 파싱·목표추출·근거페이지·본문결합·채점반영이 전부 자동으로 이어지는 경로를 구축했습니다(`python -m app.ingest.attachment_worklist`가 무엇을·어디서·어떤 이름으로 받을지 출력).

7. **불투명 4축이 일부 기업의 약속을 담지 못한다** — 척도는 ROE·환원율·기간·자사주계획을 셉니다. 그런데 SK하이닉스(CapEx/Revenue 30% 중반)·LG에너지솔루션(매출 2배·EBITDA Margin 10% 중반)은 **다른 지표로 명확히 약속**했습니다. 이들이 "순위 불가"로 표시되던 것은 부실 공시가 아니라 **우리 자에 눈금이 없는 것**이었습니다. 축을 넓히면 종목 간 비교가 흐려지므로 **못 재는 것을 못 잰다고 표시하는 쪽**을 택했습니다(목록은 "타 지표로 공시", 상세는 그 이유를 문장으로 말합니다). 유니버스 확대 후에도 해당 종목은 30건으로, 축 확장의 순수익보다 채점 계약이 갈라지는 비용이 큽니다.
