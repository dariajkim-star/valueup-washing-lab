...
class ScreeningOut(BaseModel):
    ...
    # 순위 불가의 **사유**(6.4/FR-15). `opacity_rank`가 null일 때만 값이 있다.
    # 세 값은 요약이 아니라 **행동 분기**다:
    #   undisclosed — 회사가 첨부 없이 기재했다고 선언. **신호다** → 화면이 드러낸다
    #   unreadable  — 본문 밖(첨부)에 계획이 있다. **우리 한계** → 첨부 워크리스트로
    #   unstated    — 어느 근거도 없다. **판정 보류** → 아무 말도 하지 않는다
    # 셋째를 첫째로 접으면 근거 없이 "회사가 안 냈다"고 말하게 된다(NFR2 위반).
    # null은 **두 가지**다 — 순위 대상이거나(축을 공시함), 밸류업 계획 자체가 없거나
    # (네이버 실측: rank·reason 둘 다 null). 호출부는 `opacity_rank is None`일 때만
    # 이 값을 읽으므로 모호하지 않다. 계획 부재를 '사유'로 만들지 않는 것이 의도다 —
    # 참여하지 않은 기업을 벌하지 않는다(opacity_engine.run의 모집단 규칙과 같은 결).
...
class GapAnalysisOut(BaseModel):
    ...
    plan_body_signal: str | None = None
    # 회사가 본문에서 첨부가 없다고 **직접 선언**했는가(0031). 위 신호와 직교한 사실이라
    # 별도 필드다 — 축을 공시하고도 첨부는 안 붙인 회사가 실측 102건이다. null = 미판정.
    plan_attachment_absent: bool | None = None
    # 순위 불가의 **사유**(6.4/FR-15). `opacity_rank`가 null일 때만 값이 있다.
    # 세 값은 요약이 아니라 **행동 분기**다:
    #   undisclosed — 회사가 첨부 없이 기재했다고 선언. **신호다** → 화면이 드러낸다
    #   unreadable  — 본문 밖(첨부)에 계획이 있다. **우리 한계** → 첨부 워크리스트로
