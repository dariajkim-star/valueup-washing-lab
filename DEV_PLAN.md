...
- 문서 버전: v0.2
- 작성일: 2026-07-08 · **갱신 2026-08-06**
- 담당: daria.j.kim

> **v0.2 갱신 사유**: §3 성공 지표가 **은퇴한 지표(워싱 플래그)를 4개월째 싣고 있었다.**
> 2026-07-23 실측에서 `washing_flag=True`가 0건 — 켜질 수 없는 경고등으로 드러나 은퇴했고
> (epic-6), 덱·포트폴리오는 2026-08-04에 고쳤으나 **이 문서는 빠졌다.**
> 도구가 사실이 아닌 것을 말하는 것과 같은 계열의 사고로 취급한다.
...
- 자연어 공시 원문 요약/LLM 해석 (v2 백로그).
  - **2026-08-06 진입**: `experiments/`에서 본문 신호 분류기(임베딩) 착수.
    **v2 별도 트랙이며 v1에 편입하지 않는다** — 편입하면 시스템 TRL이 최저 부품을 따라
    6에서 3으로 내려간다(파티 판정). `app/` 무변경이 유지 조건이고,
    두 트랙 사이 이동은 **실험→v1 버그 리포트 방향만** 허용한다.
...
| 지표 | 정의 | 상태 |
|---|---|---|
| **달성률(Achievement)** | 실제 ROE / 목표 ROE (배당성향·총주주환원율 동일) | 운영 중 |
| **진척률(Progress)** | (경과 기간 대비) 목표 궤도 도달 정도 | 운영 중 |
| **밸류업 실행 점수**(`execution_score`) | 위 지표 가중합 0~100 | 운영 중 |
| **불투명도 순위**(`opacity_rank`) | peer 공시 격차를 proxy로 측정한 **순위 전용** 지표 | 운영 중 (워싱 플래그의 대체) |
| ~~**워싱 플래그**~~ | ~~목표기간 50%+ 경과 & 달성률 < 임계치 & 자사주/배당 실이행 없음~~ | **은퇴 (2026-07-23)** |

> **워싱 플래그 은퇴 경위**: 실데이터에서 `washing_flag=True`가 **0건**으로 나왔다.
> 임계치를 조정해 켜지게 만들 수도 있었으나, 그것은 "걸러낼 기업이 있다"는 결론을
> 먼저 정해두고 기준을 맞추는 것이라 프로젝트 서명(*불확실을 확실로 세탁하지 않는다*)에
> 정면으로 어긋난다. 지표를 은퇴시키고 그 자리를 `opacity_rank`가 대신한다 —
> **유죄 판정이 아니라 격차 표시**로 문제를 다시 세운 것이다(레아 원칙:
> "고의를 판정하지 말고 격차를 드러내라").
>
> 이 은퇴는 손실이 아니라 산출물이다. 발표에서도 *"33종목 중 워싱확정 0건"*(방어적 해명)이
> 아니라 *"워싱 플래그를 은퇴시켰다"*(자기검증)로 말한다.
