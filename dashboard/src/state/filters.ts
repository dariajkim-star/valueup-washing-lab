...
// "불투명 상위만" 토글의 임계값. **화면에 드러나는 상수로 둔다** — 임계 소유권(config 고정
// vs 화면 다이얼)이 아직 미결이라, 숨긴 채로 굳히지 않는다. 라벨에 상위 %를 함께 표기해
// 사용자가 무엇으로 걸러졌는지 알 수 있게 한다.
export const OPAQUE_MIN_RANK = 0.7;
...
export interface FilterState {
  ...
  maxDebtRatio?: number; // %
  opaqueOnly: boolean; // 불투명 상위만 보기(구 washingOnly)
...
export const useFilters = create<FilterState>((set) => ({
  ...
  setOpaqueOnly: (opaqueOnly) => set({ opaqueOnly, page: 1 }),
...
export function toParams(s: FilterState): ScreeningParams {
  ...
  return {
    ...
    min_opacity_rank: s.opaqueOnly ? OPAQUE_MIN_RANK : undefined,
