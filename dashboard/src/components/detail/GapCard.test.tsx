...
function gap(partial: Partial<GapDetail>): GapDetail {
  return {
    ...
    plan_body_signal: "axis_targets", plan_attachment_absent: null,
...
describe("출처 표기(0015) — 목표값의 신선도를 감추지 않는다", () => {
  ...
  it("첨부 부존재는 다른 신호에 가려지지 않는다", () => {
    render(<GapCard gap={gap({
      plan_body_signal: "other_metric", plan_attachment_absent: true,
    })} />);
    expect(screen.getByText(/우리가 재는 4축.*밖의 지표/)).toBeTruthy();
  ...
  it("축을 다 공시한 회사도 첨부 부존재는 말한다 — 실측 102건이 이 조합", () => {
    render(<GapCard gap={gap({
      plan_body_signal: "axis_targets", plan_attachment_absent: true,
    })} />);
    expect(screen.getByText(/받아올 계획서 문서가 존재하지 않는다/)).toBeTruthy();
  });
